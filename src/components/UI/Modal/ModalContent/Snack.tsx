...
const Snack = () => {
  ...
  const item = getShopItemByType(shop, "snack", "productivityItems");
  ...
  const handlePurchase = useCallback(async () => {
    ...
    if (item?.toolId) {
      try {
        ...
        showToast(
          "success",
          `You've purchased ${count} ${count > 1 ? "Snacks" : "Snack"}.`
        );
      } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-start w-full gap-6">
      <div className="flex flex-col items-center justify-start w-full gap-4">
        ...
        >
          <div className=" border-3 border-[#F0DAD2] bg-[#FFFBF9] aspect-square w-[18.35vw] max-w-[76px] mx-auto z-10 flex flex-col justify-center items-center rounded-[14px]">
        ...
        <div className="flex flex-col items-center justify-start w-full gap-6">
          <p className="text-[1em] text-center font-josefin  font-medium tracking-tighter leading-5">
            {getFormatedText(
              localization["item_description.snack"],
              "5",
      ...
      <Button2
        acent="green"
        className="flex-1 w-full text-[1.25em] p-2.5"
