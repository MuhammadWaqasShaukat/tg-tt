...
const ImprovedAlarmRow = () => {
  const [, setCurrentModal] = useRecoilState(CurrentModalState);

  const improvedAlarmModal = () => {
    setCurrentModal("improved-alarm");
  };
  return (
    <div className="pt-4 flex flex-row justify-between items-center w-full border-t border-[#5A1319] border-opacity-20 gap-3 max-h-[58px]">
      ...
      <span className="text-xl font-bold text-center leading-4 tracking-tight font-josefin px-2">
        5
      </span>
      <Button3
        onClick={improvedAlarmModal}
        acent="green"
        className=" w-max"
      ></Button3>
    </div>
...
export default ImprovedAlarmRow;
